...
extern(C) @system nothrow @nogc
{
        ...
        with(hll)
        {
            if (_temp) //temp is always allocated for sparse representation
            {
                ...
                if (temp.length >= normal_length(p) / 4)
                {
                    ...
                    if (_sparse.length + _temp.length * ulong.sizeof >= normal_allocation_size(p))
                    {
    ...
    ulong dlang_hll_count(ref HLL hll)
    {
        with(hll)
        {
            ...
            import std.range;
            size_t lb = raw.bs_lower_bound(e);
            double bias = void;
            if (lb == 0)
                bias = data[0];
            else
            ...
            else
            {
                auto w1 = raw[lb] - e;
                auto w2 = e - raw[lb-1];
                bias = (data[lb] * w1 + data[lb-1] * w2) / (w1 + w2);
            }
...
private size_t bs_lower_bound()(in double[] a, double x) {
    size_t l = 0;
    size_t h = a.length;
    while (l < h) {
        size_t mid = (l + h) / 2;
...
struct HLL
{
    ...
    static nothrow @trusted size_t __xtoHash(ref const(typeof(this)))
    {
        assert(0);
    }

    static bool __xopEquals (ref const(const(typeof(this))), ref const(const(typeof(this))))
    {
        assert(0);
    }
...
pragma(inline, true):
        ...
        )
    {
        ...
        auto r = dlang_hll_create(this, p, pPrime, malloc, realloc, free);
        assert(r == 0);
    }
    ...
    private void putTemp()(ulong x)
    {
        ...
        {
            auto d = (cast(void*)_temp.ptr)[0..temp.length * ulong.sizeof];
            assert(d.ptr);
...
unittest
{
    ...
    auto input = InputSparse(d.ptr, o.count);
...
ulong fromVaruint()(ref const(ubyte)* d)
{
    ...
    size_t offset;
    ulong val = *data++;
    while (val & ~ulong(byte.max))
    {
        ret ^= (val & byte.max) << offset;
        offset += 7;
        val = *data++;
    }
    assert(offset < 64);
...
void toVaruint()(ref ubyte* d, ulong n)
{
    ...
    while (n > byte.max)
    {
        *data++ = cast(ubyte)((n & byte.max) ^ ~ulong(byte.max));
        n >>= 7;
    }
    *data++ = cast(ubyte) n;
    d = data;
...
{
    ulong n = 7;
    assert(varintLength(n) == 1);
    ubyte[2] variant;
    auto q = variant.ptr;
    toVaruint(q, n);
    assert(variant == [7, 0]);
    const(ubyte)* p = variant.ptr;
    assert(fromVaruint(p) == 7);
    assert(p == variant.ptr + 1);
}

unittest
{
    ulong n = 300;
    assert(varintLength(n) == 2);
    ubyte[2] variant;
    auto q = variant.ptr;
    toVaruint(q, n);
    assert(variant == [0b1010_1100, 0b0000_0010]);
    const(ubyte)* p = variant.ptr;
    assert(fromVaruint(p) == 300);
...
immutable double[][] _rawEstimateData = [
